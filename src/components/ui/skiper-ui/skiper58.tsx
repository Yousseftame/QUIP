...
type TextRollProps = {
  ...
  className?: string;
  center?: boolean;
  /** Inherit hover state from a parent motion element (e.g. the nav button). */
  useParentHover?: boolean;
  /** Controlled animation state when useParentHover is true. */
...
const TextRoll: React.FC<TextRollProps> = ({
  ...
  const hoverProps = useParentHover
    ? { animate }
    : { initial: "initial" as const, whileHover: "hovered" as const };
  ...
  return (
    <motion.span
      ...
      style={{
        lineHeight: useParentHover ? 1.1 : 0.75,
      }}
    >
      <div>
